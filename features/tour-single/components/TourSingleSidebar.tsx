...
export default function TourSingleSidebar({ tourContent }: TourSingleSidebarProps) {
  ...
  const extraCharge =
    (isExtraService ? prices.extraService : 0) + (isServicePerPerson ? prices.servicePerPerson : 0);
    ...
    <div className='tourSingleSidebar'>
                ...
                <div className='searchFormItemDropdown__list sroll-bar-1'>
                  {timeSlots.map((elm, i) => (
                    <div
                      key={i}
                      onClick={() => {
                        setSelectedTime((previousValue) => (previousValue === elm ? '' : elm));
      ...
      <div className='mt-15'>
        <div className='d-flex items-center justify-between'>
          ...
          <div className='d-flex items-center js-counter'>
            <button
              onClick={() => setChildrenNumber((pre) => (pre > 1 ? pre - 1 : pre))}
              className='button size-30 border-1 rounded-full js-down'
            >
              <i className='icon-minus text-10'></i>
            </button>
