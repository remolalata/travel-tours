...
interface MainTriggerIconProps {
  isOpen: boolean;
}

function MainTriggerIcon({ isOpen }: MainTriggerIconProps) {
  if (!isOpen) {
...
type ChannelButtonCandidate = Omit<ChannelButton, 'links'> & {
  links: ContactChannelLinks | null;
};
...
function MessengerButton({
  ...
  const handleChannelClick = useCallback(
    (event: ReactMouseEvent<HTMLAnchorElement>, appUrl: string, webUrl: string) => {
      if (!appUrl || !webUrl) {
        return;
      }
  ...
  const channelButtons = useMemo<ChannelButton[]>(() => {
    const candidates: ChannelButtonCandidate[] = [
      {
        id: 'messenger',
    ...
    return candidates.filter((item): item is ChannelButton => item.links !== null);
  ...
  useEffect(() => {
    ...
    const onDocumentPointerDown = (event: PointerEvent) => {
      ...
      if (
        ...
        !groupRef.current?.contains(eventTarget)
      ) {
        setIsOpen(false);
      ...
      <div id={menuId} className='contactButtonList' role='menu' aria-hidden={!isOpen}>
        {channelButtons.map((channel, index) => (
          <a
            ...
            onClick={(event) =>
              handleChannelClick(event, channel.links.appUrl, channel.links.webUrl)
            }
